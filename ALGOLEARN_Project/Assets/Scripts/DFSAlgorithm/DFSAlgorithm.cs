...
public class DFSAlgorithm : MonoBehaviour
{
    ...
    public void Algorithm()
    {
        ...
        {
            if (maze.currentRow > 0 && !maze.mazeGrid[maze.currentRow - 1, maze.currentCol].VisitedCell && boundries() != 5) // if the current row is greater then 0 and the cell above is not visited and it its not the top row 
            {
        ...
        if (Input.GetKeyDown(KeyCode.DownArrow) || (maze.solveForPlayer == true && direction == 1))
        {
            ...
            if (maze.currentRow < maze.Rows - 1 && !maze.mazeGrid[maze.currentRow + 1, maze.currentCol].VisitedCell && boundries() != 6)// if the current row is less then row max value and the cell below is not visited and it its not the bottom row 
            {
        ...
        if (Input.GetKeyDown(KeyCode.LeftArrow) || (maze.solveForPlayer == true && direction == 2))
        {
            ...
            if (maze.currentCol > 0 && !maze.mazeGrid[maze.currentRow, maze.currentCol - 1].VisitedCell && boundries() != 7) // if the current column is greater then 0 value and the cell to the left is not visited and it its not the left side 
            {
        ...
        if (Input.GetKeyDown(KeyCode.RightArrow) || (maze.solveForPlayer == true && direction == 3))
        {
            ...
            if (maze.currentCol < maze.Cols - 1 && !maze.mazeGrid[maze.currentRow, maze.currentCol + 1].VisitedCell && boundries() != 8) // if the current column is less then most right col value and the cell to the right is not visited and it its not the right side 
            {
    ...
    {
        if (boundries() == 1) // if 1 gets returned from boundries method it means the algorithm is in the top left corner so no need to check for cells to the left or cells to above
        ...
        if (boundries() == 2)// if 2 gets returned from boundries method it means the algorithm is in the bottom left corner so no need to check for cells to the left or cells down
        ...
        if (boundries() == 3)// if 3 gets returned from boundries method it means the algorithm is in the top righ corner so no need to check for cells to the right or cells above
        ...
        if (boundries() == 4)// if 4 gets returned from boundries method it means the algorithm is in the botom righ corner so no need to check for cells to the right or cells down
        ...
        //Row 0
        if (boundries() == 5)// if 5 gets returned from boundries method it means the algorithm is in the top row and it will not check the cell above
        {
        ...
        //Rows - 1
        if (boundries() == 6)// if 6 gets returned from boundries method it means the algorithm is in the bottom row and it will not check the cell below
        {
            if ((maze.mazeGrid[maze.currentRow, maze.currentCol].VisitedCell == true) &&
                (maze.mazeGrid[maze.currentRow - 1, maze.currentCol].VisitedCell == true &&
                maze.mazeGrid[maze.currentRow, maze.currentCol + 1].VisitedCell == true && maze.mazeGrid[maze.currentRow, maze.currentCol - 1].VisitedCell == true) && maze.trackX.Count != 0)
        ...
        //col 0
        if (boundries() == 7)// if 6 gets returned from boundries method it means the algorithm is in the left most cell and it will not check the cell left
        {
            if ((maze.mazeGrid[maze.currentRow, maze.currentCol].VisitedCell == true) &&
                (maze.mazeGrid[maze.currentRow + 1, maze.currentCol].VisitedCell == true &&
                maze.mazeGrid[maze.currentRow - 1, maze.currentCol].VisitedCell == true && maze.mazeGrid[maze.currentRow, maze.currentCol + 1].VisitedCell == true) && maze.trackX.Count != 0)
        ...
        //cols - 1
        if (boundries() == 8)// if 8 gets returned from boundries method it means the algorithm is in the right most cell and it will not check the cell right
        {
            if ((maze.mazeGrid[maze.currentRow, maze.currentCol].VisitedCell == true) && 
                (maze.mazeGrid[maze.currentRow + 1, maze.currentCol].VisitedCell == true &&
                maze.mazeGrid[maze.currentRow - 1, maze.currentCol].VisitedCell == true && maze.mazeGrid[maze.currentRow, maze.currentCol - 1].VisitedCell == true) && maze.trackX.Count != 0)
        ...
        if (boundries() == 0)// if 0 gets returned from boundries method it means the algorithm is not near the boundries and will check for visited cells in every direction
        {
    ...
    public int boundries()
    {
        ...
        if (maze.currentRow == 0 && maze.currentCol == 0) // check if current row and column are at 0 which is top left
        {
            return 1;
        }
        if (maze.currentRow == maze.Rows - 1 && maze.currentCol == 0) // check if current row and column is bottom letf
        {
            return 2;
        }
        if (maze.currentRow == 0 && maze.currentCol == maze.Cols - 1) // check if current row and column is top right
        {
            return 3;
        }
        if (maze.currentRow == maze.Rows - 1 && maze.currentCol == maze.Cols - 1) // check if current row and column is bottom right
        {
            return 4;
        }
        // the next 4 check if the current row is at the boundries of the maze
        if (maze.currentRow == 0) // check if the current row is the top row
        {
            return 5;
        }
        if (maze.currentRow == maze.Rows - 1) // check if the current row is the bottom row
        {
            return 6;
        }
        if (maze.currentCol == 0) // check if the current col is the left size
        {
            return 7;
        }
        if (maze.currentCol == maze.Cols - 1) // check if the current column is the right size
        {
            return 8;
        }
        return 0;
